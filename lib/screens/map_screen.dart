...
class _MapScreenState extends State<MapScreen> {
  ...
  LatLng? _getTriggerPoint(
      List<LatLng> points, LatLng target, double triggerDist) {
    ...
    for (int i = 0; i < points.length; i++) {
      double d = Geolocator.distanceBetween(points[i].latitude,
          points[i].longitude, target.latitude, target.longitude);
      if (d < minTargetDist) {
        minTargetDist = d;
        targetIdx = i;
      }
    ...
    for (int i = targetIdx - 1; i >= 0; i--) {
      ...
      if (d > triggerDist) {
        ...
        double dNext = Geolocator.distanceBetween(points[i + 1].latitude,
            points[i + 1].longitude, target.latitude, target.longitude);
      ...
      for (var step in stepsToShow) {
        ...
        if (widget.focusStep != null &&
          ...
          if (step.path != null && step.path!.isNotEmpty) {
            try {
              ...
              debugPrint("Path mapping error: $e");
            }
          } else {
            try {
              // Fallback to OSRM only if no path from Motis
              final path = await TransportApi.getWalkingRoute(
                  step.startLat!, step.startLng!, step.endLat!, step.endLng!);
        ...
        if (step.stopovers != null) {
          for (var stop in step.stopovers!) {
            if (stop['stop'] != null && stop['stop']['location'] != null) {
              markers.add(Marker(
                point: LatLng(stop['stop']['location']['latitude'],
                    stop['stop']['location']['longitude']),
                width: 10,
                height: 10,
                child: Container(
        ...
        if (step.isWakeAlarmOn) {
          ...
          if (targetLat != null && targetLng != null) {
            ...
            if (thresholdSetting.endsWith('m')) {
              try {
                triggerDist =
                    double.parse(thresholdSetting.replaceAll('m', ''));
              } catch (_) {}
              ...
              try {
                percentValue =
                    double.parse(thresholdSetting.replaceAll('%', ''));
              } catch (_) {}
              triggerDist = segmentDist * (percentValue / 100.0);
            }

            final triggerPoint = _getTriggerPoint(
                stepPoints, LatLng(targetLat, targetLng), triggerDist);
